...
public class N52_QueenSolution {
    public int totalNQueens(int n) {
        ...
        AtomicInteger numOfSolutions = new AtomicInteger(0);
        placeQueenOfNum(1, n, board, numOfSolutions);

        return numOfSolutions.get();
    }

    private void placeQueenOfNum(int noOfQueen, int n, Board board, AtomicInteger numOfSolutions) {
        if (noOfQueen > n) {
            numOfSolutions.incrementAndGet();
            return;
        }
        ...
        for (int col = 0; col < n; ++col) {
            if (board.canPlaceQueenAt(row, col)) {
                ...
                placeQueenOfNum(noOfQueen + 1, n, board, numOfSolutions);
                board.reset(row, col);
            }
        }
    }
